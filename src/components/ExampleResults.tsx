...
export default function ExampleResults({ toolId }: ExampleResultsProps) {
  ...
  useEffect(() => {
    async function loadResults() {
      try {
        ...
        console.error('加载保存的图片结果失败:', error);
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-white mb-6">示例结果</h2>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 border border-gray-800 rounded-xl bg-gray-900">
          ...
          <p className="text-gray-400">暂无示例图片</p>
          <p className="text-gray-600 text-sm mt-2">生成一些图片后将在这里显示</p>
        </div>
      ) : (
